...
const DESCRICAO_PADRAO = [
  'Painel', 'Trio de Cilindro', 'Boleiras',
  'Vaso Romano','Flores','Tapete', 'Tudo como na imagem ou nas cores que desejar'
];
...
function criarCard(tema) {
  ...
  for (var i = 1; i <= qtdImagens; i++) {
    htmlImgs +=
      '<img src="./festa-pegmonte/' + pasta + '/' + i + '.webp"' +
      ' loading="lazy"' +
      ' data-indice="' + i + '"' +
      ' onerror="if(!this.dataset.tentouMaiusculo){this.dataset.tentouMaiusculo=\'1\'; this.src=\'./Festa-Pegmonte/' + pasta + '/' + i + '.WEBP\';}else{this.dataset.falhou=\'1\';this.style.display=\'none\';}"' +
